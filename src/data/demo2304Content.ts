// Curated demo content for /demo-2304-{subject}
// Used ONLY by Demo2304.tsx — does NOT affect live student flow.
// Each subject now exposes MULTIPLE episodes:
//   - Episode 1 = INTRO (gentle hook, definition reveal). Already lean.
//   - Episode 2 = NON-INTRO (skips definition; jumps to applied challenge,
//     method mastery, cross-link mastery — heavier reasoning, no redundancy).
...
// ── Episode 2 (NON-INTRO) shapes ────────────────────────────────
// Day 1 Spark = APPLIED CHALLENGE first, reveal method only after.
export interface Ep2Day1Content {
  /** A concrete problem the student tries BEFORE being taught. */
  challengePrompt: string;
  /** Optional concrete numbers / mini-context shown alongside. */
  challengeContext?: string;
  /** What "good" looks like — used to score student attempt + Buddy hints. */
  expectedAnswer: string;
  /** The actual method/algorithm revealed AFTER the attempt. */
  methodTitle: string;
  methodBody: string;
  /** A common-misconception detective. */
  detective: DetectiveQ;
}

// Day 2 Build = METHOD MASTERY: apply the just-learned method to a harder
// case and detect a STEP-LEVEL trap, instead of redefining the concept.
export interface Ep2Day2Content {
  recallLine: string;
  /** A harder pair / scenario the student must work through. */
  applyPrompt: string;
  applyContext?: string;
  expectedAnswer: string;
  /** Pre-laid steps with ONE intentional error — student must spot it. */
  trapSteps: string[];
  trapWrongIndex: number;
  trapExplain: string;
  detective: DetectiveQ;
}

// Day 3 Master = CROSS-LINK: connect today's idea to other ideas
// (no re-explaining the concept — pure synthesis & edge cases).
export interface Ep2Day3Content {
  synthesisTitle: string;
  /** The bridge to another concept (e.g. HCF↔LCM↔prime factorization). */
  synthesisBody: string;
  /** A reasoning prompt that requires combining ideas. */
  defendPrompt: string;
  expectedAnswer: string;
  /** Edge / JEE-style case to test mastery. */
  edgeCase: { prompt: string; answer: string; explain: string };
  growth: { label: string; emoji: string; pct: number }[];
}

export interface DemoEpisode1 {
  kind: "intro";
  episodeTitle: string;
...
export type DemoEpisode = DemoEpisode1 | DemoEpisode2;
...
export const DEMO_2304: Record<Subject, DemoSubjectContent> = {
  math: {
    ...
    episodes: {
      ep1: {
        kind: "intro",
        episodeTitle: "Episode 1 · Introduction to Real Numbers",
        episodeSubtitle: "What are real numbers — and why do they matter?",
        estimatedMinutes: { d1: 4, d2: 6, d3: 5 },
        day1: {
          hookQuestion:
            "If 1, 2, 3… are 'natural', what makes 0, −5, ½ and √2 deserve to be called numbers too?",
          conceptText:
        ...
        day2: {
          recallPrompt:
            "Yesterday you said real numbers fill the line. Today: WHY can we trust there are no gaps?",
          ...
          detective: {
            statement:
              "Between 0.999… (repeating forever) and 1, there is at least one other real number.",
            isTrue: false,
            explain:
      ...
      // ── Episode 2 — NOT an intro. Builds on Ep 1's "real numbers exist on a line"
      // and zooms into the deepest tool of the chapter: Euclid's Division Lemma + HCF.
      ep2: {
        kind: "non-intro",
        episodeTitle: "Episode 2 · Euclid's Division Lemma & HCF",
        episodeSubtitle:
          "From an ancient trick to the fastest way to find a Highest Common Factor.",
        estimatedMinutes: { d1: 6, d2: 7, d3: 6 },

        day1: {
          // SPARK = applied challenge first; method only revealed AFTER the attempt.
          challengePrompt:
            "Find the HCF (highest common factor) of 56 and 72 — but solve it WITHOUT writing prime factorisation. Try any shortcut you can think of.",
          challengeContext:
            "Hint to try first: subtract the smaller from the bigger, repeat. What stays at the end?",
          expectedAnswer:
            "HCF = 8. (One way: 72 − 56 = 16, then 56 − 16 − 16 − 16 = 8, then 16 − 8 − 8 = 0. The last non-zero leftover is 8.)",
          methodTitle: "Euclid's Division Lemma — the cleanest version",
          methodBody:
            "For any two positive integers a and b (a > b), there exist unique integers q and r such that a = bq + r where 0 ≤ r < b. Apply it repeatedly:\n\n• 72 = 56·1 + 16   (remainder 16)\n• 56 = 16·3 + 8    (remainder 8)\n• 16 = 8·2 + 0     (remainder 0 → STOP)\n\nThe LAST non-zero remainder is the HCF. Here HCF(72, 56) = 8. This is Euclid's algorithm — it works for ANY pair of integers, no matter how large.",
          detective: {
            statement:
              "If you keep dividing and the remainder never reaches 0, then the two numbers have no HCF.",
            isTrue: false,
            explain:
              "False. For positive integers, the remainder is strictly DECREASING and bounded below by 0 — so it must reach 0 in a finite number of steps. Euclid's algorithm always terminates.",
          },
        },

        day2: {
          // BUILD = apply the method to a HARDER pair + spot the wrong step.
          recallLine:
            "Yesterday you used Euclid's lemma on (72, 56). Today: a harder pair — and a sneaky wrong step.",
          applyPrompt:
            "Use Euclid's algorithm to find HCF(420, 272). Write the chain of equations a = bq + r.",
          expectedAnswer:
            "420 = 272·1 + 148 → 272 = 148·1 + 124 → 148 = 124·1 + 24 → 124 = 24·5 + 4 → 24 = 4·6 + 0. HCF = 4.",
          // ONE step below is wrong on purpose — student must spot it.
          trapSteps: [
            "420 = 272·1 + 148",
            "272 = 148·1 + 124",
            "148 = 124·1 + 24",
            "124 = 24·4 + 28", // ← WRONG: 24·4 = 96, 124 − 96 = 28, but r must be < divisor (24). So q should be 5, r = 4.
            "24 = 28·0 + 24",
            "HCF = 24",
          ],
          trapWrongIndex: 3,
          trapExplain:
            "Step 4 breaks the rule 0 ≤ r < b. With divisor 24, the remainder MUST be less than 24 — but here it's 28. The correct step is 124 = 24·5 + 4. Continue: 24 = 4·6 + 0 → HCF = 4 (not 24).",
          detective: {
            statement:
              "Swapping the order — applying Euclid's algorithm to (272, 420) instead of (420, 272) — would give a different HCF.",
            isTrue: false,
            explain:
              "False. The first step would just be 272 = 420·0 + 272, which immediately flips you back to (420, 272). HCF is symmetric: HCF(a, b) = HCF(b, a).",
          },
        },

        day3: {
          // MASTER = synthesis with another concept + edge case (no re-teaching).
          synthesisTitle: "The bridge: HCF, LCM, and prime factorisation",
          synthesisBody:
            "Three different routes — Euclid's algorithm, prime factorisation, and the LCM formula — all describe the SAME deep fact: every integer has a unique prime fingerprint (Fundamental Theorem of Arithmetic).\n\nThat's why the identity HCF(a, b) × LCM(a, b) = a × b is not a coincidence. It just says: the shared primes (HCF) times the union of all primes (LCM) gives back every prime in both numbers (a × b).\n\nSo Euclid's algorithm is a fast way to compute HCF without ever finding the primes — useful when numbers are huge.",
          defendPrompt:
            "Given HCF(a, b) = 12 and LCM(a, b) = 180, can a and b both be 60? Defend your answer using the identity HCF × LCM = a × b.",
          expectedAnswer:
            "No. HCF × LCM = 12 × 180 = 2160. If a = b = 60, then a × b = 3600 ≠ 2160. So (60, 60) is impossible. (One valid pair is a = 60, b = 36: 60·36 = 2160 ✓.)",
          edgeCase: {
            prompt:
              "What is HCF(0, 17)? Most calculators just say 17 — is that actually correct?",
            answer: "17",
            explain:
              "Yes, HCF(0, n) = n for any positive n. Reason: every positive integer divides 0 (since 0 = n·0), and the largest divisor of 17 is 17 itself. Euclid's lemma confirms it: 17 = 0·q + 17 stops at remainder 17 immediately, so on the first 'real' step we get HCF = 17.",
          },
          growth: [
            { label: "Method Mastery", emoji: "🧮", pct: 18 },
            { label: "Reasoning", emoji: "🧠", pct: 14 },
            { label: "Synthesis", emoji: "🔗", pct: 11 },
          ],
        },
      },
    },
  ...
  physics: {
    ...
    episodes: {
      ep1: {
        kind: "intro",
        episodeTitle: "Episode 1 · Electric Current and Circuit",
        ...
        day1: {
          ...
          detective: {
            statement:
              "If you cut a wire in the middle of a working circuit, the bulb keeps glowing for a moment because the electrons inside the wire keep moving.",
            isTrue: false,
            explain:
        ...
        day2: {
          ...
          detective: {
            statement:
              "Doubling the voltage of a battery in the same circuit roughly doubles the current.",
            isTrue: true,
            explain:
  ...
  chemistry: {
    ...
    episodes: {
      ep1: {
        kind: "intro",
        episodeTitle: "Episode 1 · Introduction to Chemical Reactions",
        ...
        day1: {
          ...
          detective: {
            statement:
              "Cutting an apple and the apple turning brown an hour later are both chemical reactions.",
            isTrue: false,
            explain:
          ...
          quickCheck: {
            prompt:
              "Which observation is the STRONGEST clue that a chemical reaction has occurred?",
            options: [
        ...
        day2: {
          ...
          detective: {
            statement:
              "Boiling water in a kettle is a chemical reaction because steam looks very different from liquid water.",
            isTrue: false,
            explain:
