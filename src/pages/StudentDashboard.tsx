...
import { BookOpen, Brain, Eye, Zap, Heart, TrendingUp, Flame, Star, ChevronRight, GraduationCap, Target, Lightbulb, Award } from "lucide-react";
...
const StudentDashboard = () => {
  ...
  const { data: breakthroughs } = useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase.from("student_breakthroughs").select("*").eq("user_id", user!.id).order("created_at", { ascending: false }).limit(5);
      if (error) throw error;
      return data ?? [];
    },
    enabled: !!user,
  ...
  const todayScheduleItems = useMemo(() => {
    ...
    orderedSubjects.forEach((subjectName) => {
      ...
      if (subjectName === "Science") items.push({ time: BREAKS[0].time, subject: BREAKS[0].subject, icon: BREAKS[0].icon, topic: BREAKS[0].topic, type: "break", color: "#6b7280" });
      if (subjectName === "English") items.push({ time: BREAKS[1].time, subject: BREAKS[1].subject, icon: BREAKS[1].icon, topic: BREAKS[1].topic, type: "break", color: "#6b7280" });
    });
    return items;
    ...
    <DashboardLayout role="student">
      <div className="p-4 md:p-8 max-w-[1400px] mx-auto space-y-6">
          ...
          <div className="relative z-10 flex flex-col md:flex-row items-center gap-6">
            ...
            <div className="flex-1 text-center md:text-left">
              ...
              <p className="text-white/80 text-sm mb-3">
                Your mind's core abilities — clarity, thinking, attention, momentum, and character — all growing together.
              </p>
                ...
                <div key={dim.name} className={`${dim.bg} ${dim.border} border rounded-xl p-4 card-hover-lift animate-stagger-in`}
                  ...
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <span className={`text-sm font-semibold ${dim.text} flex items-center gap-1`}>
                      {dim.name}
                      <HelpTooltip content={
                        dim.name === "Clarity" ? "How well you understand core concepts." :
                        dim.name === "Thinking" ? "Your reasoning and analytical ability." :
                        dim.name === "Attention" ? "Your focus and engagement." :
                        dim.name === "Momentum" ? "Your learning velocity." :
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="lg:col-span-2 space-y-6">
            <div className="bg-card border border-border rounded-xl p-6">
              ...
              <div className="bg-gradient-to-r from-blue-50 to-indigo-50 border border-blue-200 rounded-xl p-5 flex items-center gap-4">
                <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-blue-500 to-indigo-600 flex items-center justify-center text-white text-2xl font-bold shrink-0">1</div>
                ...
                <button onClick={() => navigate("/student/textbook/ch1")}
                ...
                {Object.entries(ELITE_METHOD_META).map(([key, method]) => {
                  ...
                  return (
                    <div key={key} className={`border rounded-xl p-4 transition-all ${
                      method.available ? "bg-card border-border hover:border-primary/30 hover:shadow-sm cursor-pointer" : "bg-muted/30 border-border opacity-60 cursor-not-allowed"
                    }`}>
          ...
          {/* Right Sidebar: Compact Schedule + Focus + Breakthroughs + Stats */}
          <div className="space-y-5">
            {/* Compact Today's Classes */}
            <div className="bg-card border border-border rounded-xl p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-base font-bold text-foreground flex items-center gap-2">📅 Today's Classes</h3>
              ...
              {loading ? (
                ...
                <div className="space-y-2">
                  {todayScheduleItems.filter(s => s.type !== "break").slice(0, 1).map((cls, i) => (
                    <div key={`now-${i}`} className="flex items-center gap-3 p-3 rounded-xl bg-primary/5 border border-primary/20">
                      <div className="w-2 h-2 rounded-full bg-primary animate-pulse shrink-0" />
                      <span className="text-lg">{cls.icon}</span>
                      <div className="flex-1 min-w-0">
                        <div className="flex items-center gap-2">
                          <span className="font-semibold text-sm text-foreground">{cls.subject}</span>
                          <span className="text-[10px] font-bold text-primary bg-primary/10 px-1.5 py-0.5 rounded-full">NOW</span>
                        </div>
                        <p className="text-xs text-muted-foreground truncate">{cls.topic}</p>
                      </div>
                      <button onClick={() => {
                        const match = findTextbookMatch(cls.topic);
                        if (match?.episodeId) navigate(`/student/textbook/${match.chapterId}/${match.episodeId}`);
                        else navigate("/student/textbook");
                      }}
                        className="text-xs font-semibold text-primary bg-primary/10 hover:bg-primary/20 px-3 py-1.5 rounded-lg cursor-pointer border-none transition-all shrink-0">
                        Open →
                      </button>
                    </div>
                  ))}
                  <div className="flex flex-wrap gap-2 mt-1">
                    {todayScheduleItems.filter(s => s.type !== "break").slice(1, 5).map((cls, i) => (
                      <div key={`next-${i}`} className="flex items-center gap-2 px-3 py-2 rounded-lg bg-muted/40 border border-border/50">
                        <span className="text-xs text-muted-foreground font-medium">{cls.time.split("–")[0]?.trim()}</span>
                        <span className="text-sm">{cls.icon}</span>
                        <span className="text-xs font-medium text-foreground">{cls.subject}</span>
                      </div>
                    ))}
                  </div>
                </div>
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              <div className="bg-gradient-to-br from-amber-50 to-orange-50 border border-amber-200 rounded-lg p-4">
                ...
                <p className="text-xs text-amber-700 mt-1">Focus on Euclid's Division Algorithm</p>
              </div>
            </div>
            ...
            <div className="bg-card border border-border rounded-xl p-5">
              ...
              <div className="space-y-3">
                {(breakthroughs ?? []).length === 0 ? (
                  <EmptyState icon={Award} title="No Breakthroughs Yet" description="Complete your first episode to earn breakthroughs!" />
                ) : (
                  (breakthroughs ?? []).map((b) => (
